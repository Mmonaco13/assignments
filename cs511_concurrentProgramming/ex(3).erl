...
is_valid(TownR) ->
    lists:all(fun (X) -> is_valid(TownR, [], X) end, maps:keys(TownR));
is_valid(_TownR,Visited,[]) ->
    true;
is_valid(TownR,Visited,Current) ->
    DD = lists:flatten(lists:map(fun (H) -> childrenOf(H,TownR) end, Current)),
    case lists:any(fun(ID) -> lists:member(ID,Visited) end, DD) of
    true -> false;
    false -> is_valid(TownR,Visited++Current,DD).
 %   New_Current = lists:filter(fun (Id) -> not(lists:member(Id,Visited)) end,DD),
 % The above line would be required for a standard BFS traversal, but is not necessary for this example. 	    
