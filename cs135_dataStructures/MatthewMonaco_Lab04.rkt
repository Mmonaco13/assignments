...
(define (intersection listA listB)
  (make-set (set-difference (append listA listB)
                  (append (set-difference listA listB) (set-difference listB listA)))
  )
  )
...
(define (disjoint? setA setB)
  (if (= (length (intersection setA setB)) 0)
      #t
      #f
      )
)
