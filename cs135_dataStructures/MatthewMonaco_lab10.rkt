...
(define (M tuplelist)
  (if (= (length tuplelist) 1)
      (cadr (car tuplelist))
      (* (cadr (car tuplelist)) (M (cdr tuplelist)))
      )
  )
...
(define (CRT-HELP el li)
  (if (= (length li) 0)
      #t
      (if (= (EuclidAlgo (cadr el) (cadr (car li))) 1)
          (CRT-HELP el (cdr li))
          #f
          )
      )
...
(define (CRT-exists? tuplelist)
  (if (= (length tuplelist) 1)
      #t
      (if (CRT-HELP (car tuplelist) (cdr tuplelist))
