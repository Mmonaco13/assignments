...
(define (convert10 number base)
  (if (= number 0)
      '()
      (append (convert10 (quotient number base) base) (list (modulo number base)) )
      )
  )
...
(define (base10 oldbase numlist)
  (if (= (length numlist) 0)
      0
      (+ (* (expt oldbase (- (length numlist) 1)) (car numlist)) (base10 oldbase (cdr numlist)))
      )
  )
...
(define (geo-prog cur_num ratio n)
  (if (= n 0)
      '()
      (append (list cur_num) (geo-prog (* cur_num ratio) ratio (- n 1) ) )
      )
  )