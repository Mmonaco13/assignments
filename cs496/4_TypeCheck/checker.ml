...
and
  ...
  | DeRef(e) ->
    ...
    in (match t1 with
    | RefType(x) -> x
    | _ -> failwith "Expected a RefType")
  | SetRef(e1,e2) ->
    let t1 = type_of_expr en e1
    in let t2 = type_of_expr en e2
    in (match t1 with
    | RefType(x) -> if t2=x then UnitType else
      failwith "Expected references to match"
    | _ -> failwith "Expected a RefType")
  ...
  | Unpair(id1, id2, def, body) ->
    let t1 = type_of_expr en def
    in (match t1 with
    | PairType(x1, x2) ->
      type_of_expr
        (extend_tenv id1 x1
          (extend_tenv id2 x2 en)) body
  ...
  | Cons(he, te) ->
    ...
    in (match t2 with
    | ListType(t3) -> if t1 = t3
      then ListType(t1)
      else failwith "Types do not match"
    | _ -> failwith "Expected a ListType")
  | Null(e) ->
    let t1 = type_of_expr en e
    in (match t1 with
    | ListType(t2) -> BoolType
    | _ -> failwith "Expected a ListType")
  | Hd(e) ->
    let t1 = type_of_expr en e
    in (match t1 with
  ...
  | NullT(t) ->
    ...
    | _ -> failwith "Expected a TreeType")
  | GetData(t) ->
    let t1 = type_of_expr en t in
    (match t1 with
      | TreeType(t2) -> t2
